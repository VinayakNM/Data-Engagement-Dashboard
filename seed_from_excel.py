...
# ── CLI ───────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description='Seed CariFin DB from Excel registration list')
parser.add_argument('--season',  type=int, default=None,
                    help='Target season year (default: most recent/active)')
parser.add_argument('--file',    type=str, default='registration_list.xlsx',
                    help='Path to Excel file (default: registration_list.xlsx)')
parser.add_argument('--event',   type=str, default='Urban Challenge',
...
def detect_date_columns(df):
    ...
    for col in df.columns:
        ...
        if len(sample) and sum(1 for v in sample if parse_date(v) is not None) / len(sample) > 0.3:
...
with app.app_context():
    ...
    for label, col in [('FIRST NAME', col_first), ('LAST NAME', col_last), ('TEAM NAME', col_team)]:
        if not col:
            print(f'ERROR: Could not find required column "{label}"\nAvailable: {list(df.columns)}')
            sys.exit(1)
    ...
    for raw in df[col_team].dropna().unique():
        ...
        if not code:
            missing_teams.add(str(raw).strip()); continue
        if code in inst_cache: continue
    ...
    if args.season:
        ...
        if not season:
            print(f'  ✗ Season {args.season} not found — create it via the admin form first')
            sys.exit(1)
    else:
        ...
        if not season:
            print('  ✗ No season found — create one via the admin form first'); sys.exit(1)
    print(f'  ✓ Season: {season.year} ({season.status})')

    print('\nLooking up event ...')
    event = Event.query.filter(Event.name.ilike(f'%{args.event}%')).first()
    if not event:
        print(f'  ✗ Event matching "{args.event}" not found — create it first'); sys.exit(1)
    ...
    for _, row in df.iterrows():
        ...
        email  = (str(row[col_email]).strip() if col_email and pd.notna(row.get(col_email)) else None)
        ...
        if not p:
            p = Participant(first_name=first, last_name=last, institution_id=inst.id,
                            email=email, birth_date=bdate, sex=sex, division=div)
            db.session.add(p); db.session.flush()
            created += 1

        reg = Registration.query.filter_by(participant_id=p.id, season_event_id=se.id).first()
        if not reg:
        ...
        for snum in stage_nums:
            ...
            if not Result.query.filter_by(registration_id=reg.id, stage_id=stage.id).first():
                db.session.add(Result(registration_id=reg.id, stage_id=stage.id,
    ...
    for inst in Institution.query.order_by(Institution.code).all():
        ...
        rc = (db.session.query(Registration).join(Participant)
              .filter(Participant.institution_id == inst.id,
                      Registration.season_event_id == se.id).count())
        if pc or rc:
            print(f'  {inst.code:6} — {pc} participants, {rc} registered season {season.year}')
