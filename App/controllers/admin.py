...
def api_get_seasons():
    ...
    return jsonify(
        [{"id": s.id, "year": s.year, "description": s.description} for s in seasons]
    )
...
@admin_bp.route(
    "/api/seasons/<int:season_id>/events", endpoint="api_list_season_events"
)
@login_required
...
def api_create_event():
    ...
    existing_se = SeasonEvent.query.filter_by(
...
# ─────────────────────────────────────────
@admin_bp.route(
    "/api/season-events/<int:se_id>",
    methods=["PUT"],
    endpoint="api_update_season_event",
)
@admin_required
...
@admin_bp.route(
    "/api/season-events/<int:se_id>/status",
    methods=["PATCH"],
    endpoint="api_set_season_event_status",
)
@admin_required
...
@admin_bp.route(
    "/api/season-events/<int:se_id>",
    methods=["DELETE"],
    endpoint="api_delete_season_event",
)
@admin_required
...
def api_metrics():
    ...
    participated_ids = (
        db.session.query(Result.registration_id).join(Registration).distinct()
    )
    if season_id:
        participated_ids = participated_ids.join(
            SeasonEvent, Registration.season_event_id == SeasonEvent.id
    ...
    completed = Result.query.join(Registration).filter(Result.placement.isnot(None))
    if season_id:
...
def api_top_institutions():
    ...
    if season_id:
        q = q.join(SeasonEvent, Registration.season_event_id == SeasonEvent.id).filter(
    ...
    return jsonify(
        [{"name": r.name, "code": r.code, "count": r.count} for r in results]
    )
...
def api_participation_by_year():
    ...
    """
    rows = (
        db.session.query(Season.year, db.func.count(Registration.id).label("count"))
        ...
        funnel.append(
            {
                ...
                "finishers": finishers,
                "drop_pct": (
                    round((1 - finishers / registered) * 100, 1) if registered else 0
                ),
            }
        )
