...
def seed(test_app):
    with test_app.app_context():
        ...
        st1 = Stage(
            ...
            stage_number=1,
            distance="5K",
            stage_date=today - timedelta(days=50),
        )
        st2 = Stage(
            season_event_id=se_urban.id,
            stage_number=2,
            distance="5K",
            stage_date=today - timedelta(days=30),
        )
        st3 = Stage(
            season_event_id=se_urban.id,
            stage_number=3,
        ...
        r1a = Result(
            ...
            stage_id=st1.id,
            finish_time="00:25:00",
            placement=1,
        )
        r1b = Result(
            registration_id=reg1.id,
            stage_id=st2.id,
            finish_time="00:26:00",
            placement=1,
        )
        r1c = Result(
            registration_id=reg1.id,
            stage_id=st3.id,
            finish_time="00:27:00",
            placement=1,
        )
        r2a = Result(
...
class TestGetTotalParticipants:
    ...
    def test_filters_by_event(self, test_app, seed):
        with test_app.app_context():
            ...
            total = get_total_participants(
                seed["active_season_id"], event_id=seed["cross_id"]
            )
            assert total == 0
    ...
    def test_filters_by_division(self, test_app, seed):
        with test_app.app_context():
            total = get_total_participants(seed["active_season_id"], division="F3039")
...
class TestGetActiveParticipants:
    ...
    def test_inst2_active_count(self, test_app, seed):
        with test_app.app_context():
            active = get_active_participants(
                seed["active_season_id"], institution_code="SAGC"
            )
...
class TestGetStageCompletion:
    ...
    def test_returns_list_of_stages(self, test_app, seed):
        with test_app.app_context():
            completion = get_stage_completion(
                seed["active_season_id"], event_id=seed["urban_id"]
            )
...
class TestGetGenderSplit:
    ...
    def test_filters_by_institution(self, test_app, seed):
        with test_app.app_context():
            split = get_gender_split(seed["active_season_id"], institution_code="CBTT")
...
class TestGetParticipationStatusBreakdown:
    ...
    def test_total_equals_registrations(self, test_app, seed):
        with test_app.app_context():
            breakdown = get_participation_status_breakdown(seed["active_season_id"])
            total = (
                breakdown["participated"] + breakdown["no_show"] + breakdown["pending"]
            )
            assert total == 6
...
class TestAdminDashboardRoute:
    ...
    def test_dashboard_filter_by_event(self, client, seed):
        token = get_token(client, seed["admin_email"], "Admin123!")
        resp = client.get(
            f'/admin/dashboard?event={seed["urban_id"]}', headers=auth(token)
        )
        assert resp.status_code == 200
