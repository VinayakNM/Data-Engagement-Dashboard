...
from App.models import (
    ...
    SeasonEvent, Stage, Participant, Registration
)
...
def seed(test_app):
    with test_app.app_context():
        inst1 = Institution(name='Central Bank of TT', code='CBTT',
                            contact_person='John Doe',
                            contact_email='hr@cbtt.com',
                            phone='1-868-625-4835', status='active')
        inst2 = Institution(name='Sagicor', code='SAGC', status='active')
        db.session.add_all([inst1, inst2])
        db.session.flush()

        admin = Admin(firstname='Admin', lastname='User',
                      username='formadmin', email='admin@forms.com',
                      password='Admin123!')
        hr = HR(firstname='HR', lastname='CBTT', username='hr_cbtt',
                email='hr@forms.com', password='Hr123!',
                institution_id=inst1.id)
        db.session.add_all([admin, hr])
        db.session.flush()

        season = Season(year=2025, status='planning',
                        reg_open=date(2025, 1, 1), reg_close=date(2025, 2, 28),
                        start_date=date(2025, 3, 1), end_date=date(2025, 12, 31))
        ...
        se = SeasonEvent(season_id=season.id, event_id=event1.id, status='active',
                         start_date=date(2025, 3, 1), end_date=date(2025, 4, 30))
        db.session.add(se)
        db.session.flush()

        st1 = Stage(season_event_id=se.id, stage_number=1,
                    distance='5K', location="Queen's Park",
                    stage_date=date(2025, 3, 15))
        st2 = Stage(season_event_id=se.id, stage_number=2,
                    distance='5K', location='Brian Lara',
                    stage_date=date(2025, 4, 5))
        db.session.add_all([st1, st2])

        part = Participant(first_name='Jane', last_name='Doe',
                           institution_id=inst1.id)
        db.session.add(part)
        db.session.flush()
...
class TestInstitutionModel:
    ...
    def test_stores_contact_fields(self):
        i = Institution(name='Test', code='T002',
                        contact_person='Alice', contact_email='a@t.com',
    ...
    def test_get_json_has_all_fields(self):
        i = Institution(name='Test', code='T003',
                        contact_person='Bob', contact_email='b@t.com',
                        phone='123', status='inactive')
        j = i.get_json()
        for key in ('id', 'name', 'code', 'contact_person',
...
class TestSeasonModel:
    ...
    def test_stores_all_date_fields(self):
        s = Season(year=2030,
                   reg_open=date(2030, 1, 1), reg_close=date(2030, 2, 28),
                   start_date=date(2030, 3, 1), end_date=date(2030, 12, 31))
    ...
    def test_hr_without_institution_raises(self, test_app):
        with test_app.app_context():
            with pytest.raises(ValueError):
                HR(firstname='X', lastname='Y', username='xy99',
                   email='xy@test.com', password='pass', institution_id=None)
...
class TestEventIntegration:
    ...
    def test_season_event_unique_constraint(self, test_app, seed):
        with test_app.app_context():
            db.session.add(SeasonEvent(season_id=seed['season_id'],
                                       event_id=seed['event1_id']))
            with pytest.raises(Exception):
                db.session.flush()
            db.session.rollback()
...
class TestInstitutionAPI:
    ...
    def test_create_success(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.post('/api/forms/institutions', headers=auth(token),
                            json={'name': 'Republic Bank', 'code': 'REPT',
    ...
    def test_update_success(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.put(f'/api/forms/institutions/{seed["inst2_id"]}',
                           headers=auth(token),
    ...
    def test_patch_status(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.patch(f'/api/forms/institutions/{seed["inst2_id"]}/status',
                             headers=auth(token), json={'status': 'inactive'})
        assert resp.status_code == 200
        assert resp.get_json()['status'] == 'inactive'
        client.patch(f'/api/forms/institutions/{seed["inst2_id"]}/status',
                     headers=auth(token), json={'status': 'active'})

    def test_assign_and_remove_hr(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        # remove first
        client.delete(f'/api/forms/institutions/{seed["inst1_id"]}/remove-hr/{seed["hr_id"]}',
                      headers=auth(token))
        # assign
        resp = client.post(f'/api/forms/institutions/{seed["inst1_id"]}/assign-hr',
                           headers=auth(token), json={'user_id': seed['hr_id']})
        assert resp.status_code == 200
        # remove
        resp = client.delete(f'/api/forms/institutions/{seed["inst1_id"]}/remove-hr/{seed["hr_id"]}',
                             headers=auth(token))
        assert resp.status_code == 200
        # restore
        client.post(f'/api/forms/institutions/{seed["inst1_id"]}/assign-hr',
                    headers=auth(token), json={'user_id': seed['hr_id']})

    def test_delete_with_participants_blocked(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.delete(f'/api/forms/institutions/{seed["inst1_id"]}',
                              headers=auth(token))
        assert resp.status_code == 409

    def test_delete_empty_institution(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        r   = client.post('/api/forms/institutions', headers=auth(token),
...
class TestSeasonAPI:
    ...
    def test_get_returns_list_with_date_fields(self, client, seed):
        ...
        for f in ('reg_open', 'reg_close', 'start_date', 'end_date', 'status', 'events'):
            assert f in s
    ...
    def test_create_success(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.post('/api/forms/seasons', headers=auth(token),
                            json={'year': 2026, 'status': 'planning',
                                  'reg_open': '2026-01-01',
    ...
    def test_create_missing_year_fails(self, client, seed):
        ...
        assert client.post('/api/forms/seasons', headers=auth(token),
                           json={'status': 'planning'}).status_code == 400

    def test_create_duplicate_year_fails(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        assert client.post('/api/forms/seasons', headers=auth(token),
                           json={'year': 2025}).status_code == 400
    ...
    def test_update_status_and_dates(self, client, seed):
        ...
        assert resp.status_code == 200
        client.put(f'/api/forms/seasons/{seed["season_id"]}',
                   headers=auth(token), json={'status': 'planning'})
    ...
    def test_delete_season(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        r   = client.post('/api/forms/seasons', headers=auth(token),
                          json={'year': 2097})
        sid = r.get_json()['id']
...
class TestEventAPI:
    ...
    def test_get_events_for_season_includes_stages(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        data  = client.get(f'/api/forms/events?season_id={seed["season_id"]}',
    ...
    def test_create_event_success(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.post('/api/forms/events', headers=auth(token),
    ...
    def test_create_event_with_stages(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.post('/api/forms/events', headers=auth(token),
                            json={'name': 'Staged Run', 'event_type': 'run',
                                  'season_id': seed['season_id'],
                                  'stages': [
                                      {'distance': '5K', 'location': 'Park A',
                                       'stage_date': '2025-03-10'},
                                      {'distance': '3K', 'location': 'Park B',
    ...
    def test_create_missing_name_fails(self, client, seed):
        ...
        assert client.post('/api/forms/events', headers=auth(token),
                           json={'event_type': 'run'}).status_code == 400

    def test_update_event(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.put(f'/api/forms/events/{seed["event2_id"]}',
                           headers=auth(token),
    ...
    def test_update_event_replaces_stages(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        client.put(f'/api/forms/events/{seed["event1_id"]}',
                   headers=auth(token),
                   json={'season_id': seed['season_id'],
                         'stages': [{'distance': '10K', 'location': 'Venue X',
    ...
    def test_patch_status_active_inactive(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        resp  = client.patch(f'/api/forms/events/{seed["event1_id"]}/status',
                             headers=auth(token),
                             json={'season_id': seed['season_id'], 'status': 'inactive'})
        assert resp.status_code == 200
        assert resp.get_json()['status'] == 'inactive'
        client.patch(f'/api/forms/events/{seed["event1_id"]}/status',
                     headers=auth(token),
                     json={'season_id': seed['season_id'], 'status': 'active'})

    def test_patch_status_missing_season_fails(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        assert client.patch(f'/api/forms/events/{seed["event1_id"]}/status',
                            headers=auth(token),
                            json={'status': 'inactive'}).status_code == 400

    def test_delete_event(self, client, seed):
        token = get_token(client, seed['admin_email'], 'Admin123!')
        r   = client.post('/api/forms/events', headers=auth(token),
                          json={'name': 'Delete Me', 'event_type': 'walk'})
        eid = r.get_json()['id']
        assert client.delete(f'/api/forms/events/{eid}',
