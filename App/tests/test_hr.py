...
from App.models import (
    Admin, HR, Scorer, Institution, Season, Event,
    SeasonEvent, Stage, Participant, Registration, Result
)
from App.controllers.hr_controller import (
    get_hr_stats, get_available_events, register_participants
)
...
def seed(test_app):
    with test_app.app_context():
        ...
        admin = Admin(firstname='Admin', lastname='User',
                      username='hradmin', email='admin@hr.com', password='Admin123!')
        hr1   = HR(firstname='Jane', lastname='HR',
                   username='hr_fcit', email='hr@fcit.com',
                   password='Hr123!', institution_id=inst1.id)
        hr2   = HR(firstname='Bob', lastname='HR',
                   username='hr_scot', email='hr@scot.com',
                   password='Hr123!', institution_id=inst2.id)
        ...
        # SeasonEvents
        se_urban = SeasonEvent(season_id=season.id, event_id=urban.id,
                               start_date=today - timedelta(days=30),
                               end_date=today + timedelta(days=60))
        se_cross = SeasonEvent(season_id=season.id, event_id=cross.id,
        ...
        st1 = Stage(season_event_id=se_urban.id, stage_number=1,
                    distance='5K', stage_date=today - timedelta(days=20))
        ...
        p1 = Participant(first_name='Alice', last_name='Smith',
                         institution_id=inst1.id, sex='F', division='F3039',
                         email='alice@test.com')
        p2 = Participant(first_name='John', last_name='Doe',
                         institution_id=inst1.id, sex='M', division='M4049',
                         email='john@test.com')
        p3 = Participant(first_name='Mary', last_name='Jones',
                         institution_id=inst1.id, sex='F', division='F2029')
        # Participants for inst2
        p4 = Participant(first_name='Tom', last_name='Brown',
                         institution_id=inst2.id, sex='M', division='M3039')
        ...
        r1 = Result(registration_id=reg1.id, stage_id=st1.id, finish_time='00:28:15', placement=1)
        r2 = Result(registration_id=reg1.id, stage_id=st2.id, finish_time='00:29:00', placement=2)
        r3 = Result(registration_id=reg2.id, stage_id=st1.id, finish_time='00:32:45', placement=3)
...
class TestParticipantModel:
    ...
    def test_participant_get_json_has_all_fields(self):
        p = Participant(first_name='Jane', last_name='Doe', institution_id=1,
                        sex='F', division='F3039', email='jane@test.com')
        j = p.get_json()
        for key in ('id', 'first_name', 'last_name', 'sex',
                    'division', 'email', 'institution_id', 'birth_date'):
            assert key in j

    def test_participant_kwargs_set_correctly(self):
        p = Participant(first_name='X', last_name='Y', institution_id=1,
                        sex='M', division='M2029', contact='1-868-555-0000')
        assert p.sex      == 'M'
...
class TestCreateParticipant:
    ...
    def test_creates_and_returns_participant(self, test_app, seed):
        with test_app.app_context():
            p = create_participant(
                ...
                email='new@test.com', institution_id=seed['inst1_id']
            )
    ...
    def test_birth_date_string_converted(self, test_app, seed):
        with test_app.app_context():
            p = create_participant(
                first_name='Date', last_name='Test',
                email='date@test.com', institution_id=seed['inst1_id'],
    ...
    def test_invalid_birth_date_becomes_none(self, test_app, seed):
        with test_app.app_context():
            p = create_participant(
                ...
                email='bad@test.com', institution_id=seed['inst1_id'],
    ...
    def test_optional_fields_stored(self, test_app, seed):
        with test_app.app_context():
            p = create_participant(
                first_name='Full', last_name='Data',
                email='full@test.com', institution_id=seed['inst1_id'],
                sex='F', division='F4049', contact='1-868-000-0001'
            )
            assert p.sex      == 'F'
    ...
    def test_participant_persists_to_db(self, test_app, seed):
        with test_app.app_context():
            p = create_participant(
                first_name='Persist', last_name='Check',
                email='persist@test.com', institution_id=seed['inst1_id']
            )
            pid = p.id
...
class TestRegisterParticipants:
    ...
    def test_registers_participants(self, test_app, seed):
        with test_app.app_context():
            ...
            count = register_participants(
                [str(seed['p3_id'])], seed['se_urban_id']
            )
            assert count == 1
            reg = Registration.query.filter_by(
                participant_id=seed['p3_id'],
                season_event_id=seed['se_urban_id']
            ).first()
    ...
    def test_skips_duplicate_registrations(self, test_app, seed):
        with test_app.app_context():
            ...
            count = register_participants(
                [str(seed['p1_id'])], seed['se_urban_id']
            )
            assert count == 0
    ...
    def test_registers_multiple_at_once(self, test_app, seed):
        with test_app.app_context():
            ...
            count = register_participants(
                [str(pa.id), str(pb.id)], seed['se_cross_id']
            )
            assert count == 2
...
class TestAddParticipantRoute:
    ...
    def test_add_participant_post_creates_participant(self, client, seed):
        token = get_token(client, seed['hr1_email'], 'Hr123!')
        resp  = client.post('/hr/participants/add', headers=auth(token), data={
            'first_name': 'Route',
            'last_name':  'Test',
    ...
    def test_add_participant_persists(self, client, test_app, seed):
        token = get_token(client, seed['hr1_email'], 'Hr123!')
        client.post('/hr/participants/add', headers=auth(token), data={
...
class TestUploadCSVRoute:
    ...
    def test_upload_valid_csv(self, client, test_app, seed):
        ...
        resp = client.post(
            ...
            headers=auth(token),
            data={'csv_file': (
                __import__('io').BytesIO(csv_data.encode()), 'test.csv'
            )},
    ...
    def test_upload_rejects_non_csv(self, client, seed):
        ...
        resp = client.post(
            ...
            headers=auth(token),
            data={'csv_file': (
                __import__('io').BytesIO(b'data'), 'test.xlsx'
            )},
            content_type='multipart/form-data'
        )
    ...
    def test_upload_skips_duplicates(self, client, test_app, seed):
        ...
        client.post(
            ...
            headers=auth(token),
            data={'csv_file': (
                __import__('io').BytesIO(csv_data.encode()), 'dup.csv'
            )},
            content_type='multipart/form-data'
        )
        with test_app.app_context():
...
class TestRegisterRoute:
    ...
    def test_register_post_creates_registrations(self, client, test_app, seed):
        ...
        with test_app.app_context():
            fresh = create_participant('Fresh', 'Reg', 'fresh_reg@test.com',
                                       seed['inst1_id'])
            fresh_id = fresh.id

        resp = client.post('/hr/register', headers=auth(token), data={
        ...
        with test_app.app_context():
            reg = Registration.query.filter_by(
                ...
                season_event_id=seed['se_cross_id']
            ).first()
            assert reg is not None
...
class TestAuthRoutes:
    ...
    def test_api_login_success(self, client, seed):
        resp = client.post('/api/login',
                           json={'email': seed['hr1_email'], 'password': 'Hr123!'})
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_api_login_wrong_password(self, client, seed):
        resp = client.post('/api/login',
                           json={'email': seed['hr1_email'], 'password': 'wrong'})
