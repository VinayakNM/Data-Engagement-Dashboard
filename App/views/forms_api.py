...
def form_get_events():
    ...
    for e in events:
        ...
        total_stages = len(stages) or sum(
            Stage.query.filter_by(season_event_id=x.id).count() for x in e.season_events
        )
        ...
        result.append(
            {
                ...
                "start_date": (
                    se.start_date.isoformat() if se and se.start_date else None
                ),
...
def form_create_event():
    ...
    if season_id:
        se = SeasonEvent.query.filter_by(
            season_id=int(season_id), event_id=e.id
        ).first()
        if not se:
            ...
            db.session.flush()
        if "stages" in data:
            _replace_stages(se.id, data["stages"])
...
def form_update_event(eid):
    ...
    if season_id:
        ...
        # Only replace stages if explicitly provided — avoids wiping on simple assign
        if "stages" in data:
            _replace_stages(se.id, data["stages"])
...
def _replace_stages(season_event_id, stages_data):
    ...
    Stage.query.filter_by(season_event_id=season_event_id).delete(
        synchronize_session="fetch"
    )
    db.session.flush()
    for i, st in enumerate(stages_data):
        try:
            stage_number = int(st.get("stage_number") or (i + 1))
...
def form_update_season(sid):
    ...
    if "reg_open" in data:
        s.reg_open = _parse_date(data["reg_open"])
    if "reg_close" in data:
        s.reg_close = _parse_date(data["reg_close"])
    if "start_date" in data:
        s.start_date = _parse_date(data["start_date"])
    if "end_date" in data:
        s.end_date = _parse_date(data["end_date"])
...
def form_get_institutions():
    ...
    for i in insts:
        ...
        history = (
            db.session.query(S.year, func.count(R.id).label("registered"))
        ...
        if not current_season:
            current_season = (
                Season.query.filter_by(status="closed")
                .order_by(Season.year.desc())
                .first()
            )
        if not current_season:
            current_season = (
                Season.query.filter(Season.status.notin_(["planning"]))
                .order_by(Season.year.desc())
                .first()
            )
        ...
        if current_season:
            ...
            season_participant_count = (
                db.session.query(f2.count(f2.distinct(P.id)))
                .join(R, P.id == R.participant_id)
                .join(SE, R.season_event_id == SE.id)
                .filter(SE.season_id == current_season.id, P.institution_id == i.id)
                .scalar()
                or 0
            )
        ...
        result.append(
            {
                **i.get_json(),
                ...
                "participation_history": [
                    {"year": row.year, "registered": row.registered} for row in history
                ],
            }
        )
    return jsonify(result)
...
def form_update_institution(iid):
    ...
    if "name" in data:
        i.name = data["name"]
    if "code" in data:
        i.code = data["code"].strip().upper()
    if "contact_person" in data:
        i.contact_person = data["contact_person"]
    if "contact_email" in data:
        i.contact_email = data["contact_email"]
    if "phone" in data:
        i.phone = data["phone"]
    if "status" in data:
...
def form_events_list():
    ...
    events = Event.query.order_by(Event.name).all()
    return jsonify(
        [{"id": e.id, "name": e.name, "event_type": e.event_type} for e in events]
    )
...
def form_seasons_list():
    ...
    return jsonify(
        [
            {"id": s.id, "year": s.year, "status": s.status or "planning"}
            for s in seasons
        ]
    )
