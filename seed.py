...
def seed():
    ...
    with app.app_context():
        ...
        # ── SEASON-EVENT BRIDGE ───────────────────────────────────────────────
        if current_season:
            if not SeasonEvent.query.filter_by(
                season_id=current_season.id, event_id=urban.id
            ).first():
                db.session.add(
                    SeasonEvent(
            ...
            if not SeasonEvent.query.filter_by(
                season_id=current_season.id, event_id=cross.id
            ).first():
                db.session.add(
                    SeasonEvent(
                        season_id=current_season.id,
            ...
            if (
                relay
                and not SeasonEvent.query.filter_by(
        ...
        if os.path.exists(EXCEL_SEED_PATH):
            ...
            try:
                ...
                print(
                    f"  ✓ Excel import: {result['created']} participants, "
                ...
                if result["unmatched"]:
                    print(
                        f"  ⚠ Unmatched institutions: {', '.join(result['unmatched'])}"
                    )
...
def import_excel_seed(filepath, season_year):
    ...
    if not all([col_first, col_last, col_team]):
        raise ValueError(
            "Excel missing required columns: FIRST NAME, LAST NAME, TEAM/INSTITUTION"
        )
    ...
    def find_institution(raw_name):
        ...
        for inst in all_institutions:
            ...
            if icode == key or icode == key_n:
                return inst
            if inst.name.lower() in key or key in inst.name.lower():
                return inst
    ...
    if col_event:
        ...
        if not default_se:
            default_event = (
                Event.query.filter_by(name="Urban Challenge").first()
    ...
    for _, row in df.iterrows():
        try:
            ...
            se = (
                se_cache.get(
                    clean(row.get(col_event, "")).lower()
                    if col_event
                    else "__default__"
                )
                if (col_event or "__default__" in se_cache)
                else None
            )
            if not se:
                skipped += 1
